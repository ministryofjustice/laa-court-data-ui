...
RSpec.describe "hearings/_attendees_on_prosecution_cases", type: :view do
  ...
  shared_examples "returns correct headers" do
    it "displays the section" do
      expect(subject).to have_tag("h2.govuk-heading-l", text: /Attendees/)
    end

    it "displays the defendants section" do
      expect(subject).to have_tag(small_gov_heading, text: /Defendants/)
    end

    it "displays the defence section" do
      expect(subject).to have_tag(small_gov_heading, text: /Defence advocates/)
    end

    it "displays the prosecution section" do
      expect(subject).to have_tag(small_gov_heading, text: /Prosecution advocates/)
    end

    it "displays the judges section" do
      expect(subject).to have_tag(small_gov_heading, text: /Judges/)
    end
  end
  ...
  context "when hearing data is present", :stub_v2_hearing_data do
    ...
    context "with defendant_names" do
      it "displays defendant names with line breaks" do
        expect(subject).to have_tag("p.govuk-body#defendants", text: /Leon Goodwin\s*David Blaine/) do
          with_tag("br", count: 1)
        end
      end
    end
    ...
    context "with defence_counsel" do
      ...
      it "displays list of defence council" do
        ...
        expect(subject).to have_tag("p.govuk-body#defence",
                                    text: text_match)
      end
    end
    ...
    context "when defence counsel did not attend" do
      ...
      it "displays list of defence council" do
        text_match = /Mark Jones \(junior\) for Leon Goodwin.*David Williams \(junior\) for not available/
        expect(subject).not_to have_tag("p.govuk-body#defence", text: text_match)
      end
    end
    ...
    context "with prosecution_counsel" do
      ...
      it "displays list of prosecution counsel" do
        expect(subject).to have_tag("p.govuk-body#prosecution", text: /John Smith/)
      end
    end

    context "with judiciaries" do
      it "displays list of judges" do
        expect(subject).to have_tag("p.govuk-body#judges",
  ...
  context "when no hearing data is present", :stub_v2_empty_hearing_data do
    context "with no defendant_names" do
      it not_available_test do
        expect(subject).to have_tag("p.govuk-body#defendants", text: not_available_text)
      end
    end

    context "with no defence_counsel" do
      it not_available_test do
        expect(subject).to have_tag("p.govuk-body#defence", text: not_available_text)
      end
    end

    context "with no prosecution_counsel" do
      it not_available_test do
        expect(subject).to have_tag("p.govuk-body#prosecution", text: not_available_text)
      end
    end

    context "with no judiciaries" do
      it not_available_test do
        expect(subject).to have_tag("p.govuk-body#judges", text: not_available_text)
      end
    end
  end
end
