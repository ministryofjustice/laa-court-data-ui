...
RSpec.describe "Maintenance mode", type: :feature do
  context "when enabled" do
    ...
    context "when out of hours" do
      let(:specified_time) { Time.zone.local(2025, 1, 1, 22, 0, 1) }
    ...
    context "when within hours" do
      let(:specified_time) { Time.zone.local(2025, 1, 1, 7, 0, 1) }

      it "does not block the page" do
        expect(page).to have_no_text "Sorry, this service is unavailable"
      end
    end
  end
end
