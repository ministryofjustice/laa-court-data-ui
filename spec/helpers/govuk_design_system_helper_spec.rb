...
RSpec.describe GovukDesignSystemHelper, type: :helper do
  ...
  describe "#govuk_detail" do
    ...
    it "adds detail with govuk class" do
      expect(subject).to have_tag(:details, with: { class: "govuk-details" })
    end

    it "adds detail without open attribute" do
      expect(subject).not_to have_tag(:details, with: { open: "open" })
    end

    it "adds detail govuk data-module" do
      expect(subject).to have_tag(:details, with: { "data-module": "govuk-details" })
    end

    it "adds nested summary tag with govuk class" do
      expect(subject).to have_tag(:details) do
    ...
    context "when using open option" do
      shared_examples "adds details without open attribute" do
        it "adds detail without open attribute" do
          expect(subject).to have_tag(:details, without: { open: "open" })
        end
      end
      ...
      context "with open true" do
        ...
        it "adds detail with open attribute" do
          expect(subject).to have_tag(:details, with: { open: "open" })
        end
      end
    ...
    context "with custom classes" do
      ...
      it "adds detail with custom classes, prepended by govuk class" do
        expect(subject).to have_tag(:details, with: { class: "govuk-details my-custom-class1 my-custom-class2" })
      end
    end
  end
  ...
  describe "#govuk_summary_list_entry" do
    ...
    it "adds row with govuk class" do
      expect(subject).to have_tag(:div, with: { class: "govuk-summary-list__row" })
    end

    it "adds nested key in dt tag with govuk class" do
      expect(subject).to have_tag(:dt, with: { class: "govuk-summary-list__key" })
    end

    it "adds nested value in dd tag with govuk class" do
      expect(subject).to have_tag(:dd, with: { class: "govuk-summary-list__value" })
    end
  end
  ...
  describe "#govuk_notification_banner" do
    ...
    it "adds a title tag" do
      expect(subject).to have_text("Important")
    end

    it 'adds the role of "region"' do
      expect(subject).to have_tag(:div, role: "region")
    end

    it "adds the content tag" do
      expect(subject).to have_tag(:div, with: { class: "govuk-notification-banner__content" })
    end

    it "adds the text" do
      expect(subject).to have_text("Some text value")
    end

    it "does not add a body class" do
      expect(subject).not_to have_tag(:p, with: { class: "govuk-body" })
    end
    ...
    context "when extra content is provided" do
      ...
      it "adds the extra content in a body class" do
        expect(subject).to have_tag(:p, with: { class: "govuk-body" }, text: "Some content")
      end
    end
    ...
    context "when it is a success banner" do
      ...
      it "adds a title tag" do
        expect(subject).to have_text("Success")
      end

      it "adds the success class" do
        expect(subject).to have_tag(:div, with: { class: "govuk-notification-banner--success" })
      end

      it 'changes the role to "alert"' do
        expect(subject).to have_tag(:div, role: "alert")
      end
    end
    ...
    context "when it is a failure banner" do
      ...
      it "adds a title tag" do
        expect(subject).to have_text("Failure")
      end

      it "adds the failure class" do
        expect(subject).to have_tag(:div, with: { class: "govuk-notification-banner--failure" })
      end
    ...
    context "when the banner has a bespoke title" do
      ...
      it "adds a title tag" do
        expect(subject).to have_text("Something")
      end

      it "doesn't add a different class" do
        expect(subject).not_to have_tag(:div, with: { class: "govuk-notification-banner--something" })
      end
    end
  end
end
