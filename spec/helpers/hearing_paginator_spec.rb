...
RSpec.describe HearingPaginator, type: :helper do
  ...
  describe '#items' do
    ...
    context 'with multiple hearings' do
      ...
      it 'each sub array contains id at index 0' do
        ids = items.collect { |el| el[0] }
        expect(ids).to match_array(%w[hearing-uuid-1 hearing-uuid-1 hearing-uuid-2 hearing-uuid-3])
      end

      it 'each sub array contains datetime object at index 0' do
        dates = items.collect { |el| el[1] }
        expect(dates).to all(be_a(DateTime))
      end

      it 'sorts array of arrays by date' do
