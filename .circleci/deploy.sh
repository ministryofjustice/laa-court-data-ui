...
# Consider only the part of the branch name which corresponds to the Jira ticket code (e.g. LAACONNECT-1234).
# Then add "-vcd" to form the first part of the deployment host name.
# Example:
# LAACONNECT-1234-add-new-feature -> https://laaconnect-1234-vcd.apps.live.cloud-platform.service.justice.gov.uk
BRANCH_RELEASE_NAME=$(echo "$CIRCLE_BRANCH" | tr '[:upper:]' '[:lower:]' | sed 's:^\w*\/::' | tr -s ' _/[]().' '-' | cut -d'-' -f1-2)
